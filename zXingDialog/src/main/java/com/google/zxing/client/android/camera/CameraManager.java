...
import com.excelsecu.zxing.util.BitmapUtil;
...
public final class CameraManager {
	...
	// @ch scale for findDesiredDimensionInRange
	private static final float SCALE = (float) (3.0 / 8.0);
	// @ch change 1920 to 1080 for rectangle scan box
	...
	public synchronized Rect getFramingRect() {
		if (framingRect == null) {
			...
			int width = findDesiredDimensionInRange(screenResolution.x, MIN_FRAME_WIDTH,
					MAX_FRAME_WIDTH);
			// @ch change scan box from rectangle to square
			int height = findDesiredDimensionInRange(screenResolution.x, MIN_FRAME_HEIGHT,
					MAX_FRAME_HEIGHT);
		...
		// @ch other code can modify framingRect, bad
		return new Rect(framingRect);
	}

	private static int findDesiredDimensionInRange(int resolution, int hardMin, int hardMax) {
		int dim = (int) (resolution * SCALE);
		if (dim < hardMin) {
			return hardMin;
		}
		if (dim > hardMax) {
	...
	public synchronized Rect getFramingRectInPreview() {
		if (framingRectInPreview == null) {
			...
			// @ch change to vertical, cameraResolution is always landscape
		...
		// @ch other code can modify framingRectInPreview, bad
		return new Rect(framingRectInPreview);
	}
	...
	public PlanarYUVLuminanceSource buildLuminanceSource(byte[] data, int width, int height) {
		...
		// @ch rotated the landscape image
		...
		height = tmp;
		data = rotatedData;
		// @ch save the capture image
		// BitmapUtil.saveYUV420(data, width, height, rect.left, rect.top,
		// rect.width(), rect.height());
		// @ch use CaptureActivity to debug, it can show the real capture image
		// on Screen
		if (context instanceof CaptureActivity) {
			final Bitmap bitmap = Bitmap.createBitmap(
					BitmapUtil.createBitmapfromYUV420(data, width, height), rect.left, rect.top,
					rect.width(), rect.height());
