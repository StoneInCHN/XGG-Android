 ...
 */
public class MineShopDetailFragment extends BaseFragment implements ShopInfoModifyView,
    ...
    protected void initData() {
        ...
        mDownPhotoPresenter = new DownPhotoPresenterImpl(getContext(), this);
    ...
    protected void initView() {
        ...
        mAdapter = new PhotoAdapter(getUsualFragment(), mEnvPhotos);
    ...
    protected void setEvent() {
        ...
        setViewClickEvent(R.id.shop_btn_location, new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                FragmentJumpUtil.toLocationFragmentForResult(getUsualFragment(),
                        mShopInfo.getName(), mShopInfo.getLatitude(), mShopInfo.getLongitude());
...
//                                            Log.d(TAG, "onLoadFailed: ");
    ...
    private void toParseLocation() {
        ...
        options.city(mCity != null ? mCity : "");
        options.address(getInputAddress());
        //解析地址数据
    ...
    private void selectStartBusinessTime() {
        ...
        new TakeHMTimeHelper(getFragmentManager(), tvStartTime.getText().toString())
    ...
    private void selectEndBusinessTime() {
        ...
        new TakeHMTimeHelper(getFragmentManager(), tvEndTime.getText().toString())
    ...
    private void refreshUI(ResShopInfo shopInfo) {
        if (shopInfo != null) {
            ...
            //折扣
            LineViewHelper.setValue(getView(), R.id.shop_lnv_discount, String.format("%.1f折", mShopInfo.getDiscount()));
            LineViewHelper.setValue(getView(), R.id.shop_lnv_average, String.format("%.1f", mShopInfo.getAvgPrice()));
            //显示 营业时段
            ...
            LineViewHelper.setValue(getView(), R.id.shop_lnv_phone, mShopInfo.getStorePhone());//电话
            LineViewHelper.setValue(getView(), R.id.shop_lnv_name, mShopInfo.getName());//店名
            //其他服务
            ...
            if (region != null) {
                LineViewHelper.setValue(getView(), R.id.shop_lnv_location, region.getFull_name());
                //从详细地址中截取地区信息
                address = address.replace(region.getFull_name(), "");
            }
            //地址
            LineViewHelper.setValue(getView(), R.id.shop_location_lnv_address, address);//详细地址
        }
    }
    ...
    public String getAnverage() {
        return LineViewHelper.getValue(getView(), R.id.shop_lnv_average);
    }
    ...
    public String getPhone() {
        return LineViewHelper.getValue(getView(), R.id.shop_lnv_phone);
    }

    @Override
    public String getName() {
        return LineViewHelper.getValue(getView(), R.id.shop_lnv_name);
    }
    ...
    public void onGetSuggestionResult(SuggestionResult result) {
        ...
        if(point != null && (mShopInfo.getLatitude() != point.latitude || mShopInfo.getLongitude() != point.longitude)){
