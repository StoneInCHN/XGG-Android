...
public final class CaptureFragmentHandler extends Handler {
	...
	CaptureFragmentHandler(CaptureFragment dialog, Collection<BarcodeFormat> decodeFormats,
		...
		decodeThread = new DecodeThread(dialog, decodeFormats, baseHints, characterSet,
				new ViewfinderResultPointCallback(dialog.getViewfinderView2()));
		decodeThread.start();
		state = State.SUCCESS;
	...
	public void handleMessage(Message message) {
		...
		case Res.id.launch_product_query:
			...
			ResolveInfo resolveInfo = dialog.getContext().getPackageManager()
