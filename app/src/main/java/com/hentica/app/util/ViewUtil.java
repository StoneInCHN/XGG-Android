...
public class ViewUtil {
     ...
     * @param url      图片地址
     */
    public static void bindImage(AQuery query, int imgId, String url) {

        ImageView imageView = query.id(imgId).getImageView();
        Glide.with(query.getContext()).load(url).into(imageView);
    }
    ...
    /**
     * 绑定头像，因为头像很小，加载时做一定的压缩处理
     */
    public static void bindHeadImage(final View rootView, int imgId, String url, final int defaultImg){
        final ImageView imageView = ViewUtil.getHolderView(rootView, imgId);
        if(imageView == null){
            return;
        }
        Glide.with(rootView.getContext()).load(url).asBitmap().centerCrop().into(new SimpleTarget<Bitmap>(200,200) {
            @Override
            public void onResourceReady(Bitmap resource, GlideAnimation<? super Bitmap> glideAnimation) {
                if(resource != null){
                    ...
                    // resource.recycle();  可能导致还没显示就被recycle
                }else {
    ...
    public static  int getListCount(List list){
        return ArrayListUtil.isEmpty(list) ? 0 : list.size();
    }
    ...
    public static void bindEditDelete(AQuery query , int editId, final int leftDraw, final int deleteDraw){
        final EditText edit = query.id(editId).getEditText();
        edit.setOnFocusChangeListener(new View.OnFocusChangeListener() {
            @Override
            public void onFocusChange(View v, boolean hasFocus) {
                // 获得焦点
                if(hasFocus && !TextUtils.isEmpty(edit.getText()))
                    edit.setCompoundDrawablesWithIntrinsicBounds(leftDraw, 0, deleteDraw, 0);
                else {
                    // 失去焦点
                    edit.setCompoundDrawablesWithIntrinsicBounds(leftDraw,0,0,0);
                }
            }
        });
        ...
        edit.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                ...
                if(event.getAction() == MotionEvent.ACTION_UP){
                    if(drawable != null ){
                        boolean touchable = event.getX() > (edit.getWidth() - edit.getTotalPaddingRight())
                                && (event.getX() < ((edit.getWidth() - edit.getPaddingRight())));
                        if(touchable){
                            // 执行点击事件
                            edit.setText("");
                        }
    ...
    /** 输入框绑定删除按钮 */
    public static void bindEditDelete(View parentView, final int editId,final int leftDraw, final int deleteDraw){
        AQuery query = new AQuery(parentView);
        bindEditDelete(query, editId, leftDraw, deleteDraw);
    }

    /** 输入框绑定删除按钮 */
    public static void bindEditDelete(Activity activity, final int editId,final int leftDraw, final int deleteDraw){
        AQuery query = new AQuery(activity);
        bindEditDelete(query, editId, leftDraw, deleteDraw);
    }
}
